...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (Novice::IsPressMouse(0) && MP >= 1 && spearmouse == 1) {
			...
			if (frame >= 500) {
				Novice::DrawEllipse(
				  x, y + 200, -300 + frame, -300 + frame / 1.5, 0.0f, 0x732ae8FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x + 1, y + 200, -300 + frame, -300 + frame / 1.5, 0.0f, 0x860018FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x - 1, y + 200, -300 + frame, -300 + frame / 1.5, 0.0f, 0x9e001cFF,
				  kFillModeWireFrame);
			}
			if (frame >= 800) {
				Novice::DrawEllipse(
				  x, y + 200, -600 + frame, -450 + frame / 1.5, 0.0f, 0x732ae8FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x + 1, y + 200, -600 + frame, -450 + frame / 1.5, 0.0f, 0x860018FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x - 1, y + 200, -600 + frame, -450 + frame / 1.5, 0.0f, 0x9e001cFF,
				  kFillModeWireFrame);
			}
			if (frame >= 1100) {
				Novice::DrawEllipse(
				  x, y + 200, -900 + frame, -600 + frame / 1.5, 0.0f, 0x732ae8FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x + 1, y + 200, -900 + frame, -600 + frame / 1.5, 0.0f, 0x860018FF,
				  kFillModeWireFrame);
				Novice::DrawEllipse(
				  x - 1, y + 200, -900 + frame, -600 + frame / 1.5, 0.0f, 0x9e001cFF,
				  kFillModeWireFrame);
			}
			for (int i = 0; i < 36; i++) {
				x += 14;
				...
				if (frame >= 125) {
					Novice::DrawBox(
					  940 + rand() % 601 - 600, 500 + rand() % 13 - 6, 9, 9, rand() % 3 - 1,
					  0x860018FF, kFillModeSolid);
					Novice::DrawBox(
					  940 + rand() % 601 - 600, 500 + rand() % 13 - 6, 6, 6, rand() % 3 - 1,
					  0x860018FF, kFillModeSolid);
				}
				if (frame >= 312) {
					Novice::DrawBox(
					  780 + rand() % 701 - 500, 400 + rand() % 13 - 6, 11, 11, rand() % 3 - 1,
					  0x9e001cFF, kFillModeSolid);
					Novice::DrawBox(
					  780 + rand() % 701 - 500, 400 + rand() % 13 - 6, 6, 6, rand() % 3 - 1,
					  0x9e001cFF, kFillModeSolid);
				}
				if (frame >= 500) {
					Novice::DrawBox(
					  640 + rand() % 801 - 400, 300 + rand() % 13 - 6, 11, 11, rand() % 3 - 1,
					  0xce0025FF, kFillModeSolid);
					Novice::DrawBox(
					  640 + rand() % 801 - 400, 300 + rand() % 13 - 6, 8, 8, rand() % 3 - 1,
					  0xce0025FF, kFillModeSolid);
				}
				if (frame >= 2) {
					for (int j = 0; j < 5; j++) {
						// Novice::DrawEllipse(x + rand() % A - AA, y - 250 + rand() % 13 -
						// 6, 1.5, 1.5, 0.0f, BLUE, kFillModeSolid);
						Novice::DrawBox(
						  x + rand() % A - AA, y - 250 + rand() % 13 - 6, 6, 6, rand() % 3 - 1,
						  BLUE, kFillModeSolid);
					}
				}
				if (frame >= 250) {
					for (int j = 0; j < 5; j++) {
						Novice::DrawBox(
						  x + rand() % A - AA, y - 250 + rand() % 13 - 6, 9, 9, rand() % 3 - 1,
						  0x5290FFFF, kFillModeSolid);
					}
				}
				if (frame >= 375) {
					for (int j = 0; j < 5; j++) {
						Novice::DrawBox(
						  x + rand() % A - AA, y - 250 + rand() % 13 - 6, 10, 10, rand() % 3 - 1,
						  0x4ed7ffFF, kFillModeSolid);
					}
				}
				if (frame >= 500) {
					for (int j = 0; j < 5; j++) {
						// Novice::DrawEllipse(x + rand() % A - AA, y - 250 + rand() % 13 - 6, 6,
						// 6,0.0f, 0x732ae8FF, kFillModeSolid);
						Novice::DrawBox(
						  x + rand() % A - AA, y - 250 + rand() % 13 - 6, 15, 15, rand() % 3 - 1,
						  0x732ae8FF, kFillModeSolid);
					}
				}
			}
		...
		if (timer == -15) {
			start = 0;
			timer = 15;
			Novice::StopAudio(SEcount[3]);
			Novice::StopAudio(SEcount[4]);
		...
		if (start == 1) {
			...
			if (timer <= 14 && timer >= 10) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle, 1, 1, 0, WHITE);
			}
			if (timer <= 10 && timer > 5) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle1, 1, 1, 0, WHITE);
			}
			if (timer <= 5 && timer >= 0) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle2, 1, 1, 0, WHITE);
			}
			if (timer <= 0 && timer >= -5) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle3, 1, 1, 0, WHITE);
			}
			if (timer <= -5 && timer >= -10) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle4, 1, 1, 0, WHITE);
			}
		}
		...
		if (start == 2) {
			...
			if (timer <= 14 && timer >= 11) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle5, 1, 1, 0, WHITE);
			}
			if (timer <= 11 && timer > 8) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle6, 1, 1, 0, WHITE);
			}
			if (timer <= 8 && timer >= 5) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle7, 1, 1, 0, WHITE);
			}
			if (timer <= 5 && timer >= 2) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle8, 1, 1, 0, WHITE);
			}
			if (timer <= -1 && timer >= -4) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle9, 1, 1, 0, WHITE);
			}
			if (timer <= -4 && timer >= -7) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle10, 1, 1, 0, WHITE);
			}
			if (timer <= -7 && timer >= -10) {
				Novice::DrawSprite(
				  effect.position.x, effect.position.y, graphhandle11, 1, 1, 0, WHITE);
			}
		}
		Novice::ScreenPrintf(10, 10, "Mouse_left : start");
		Novice::ScreenPrintf(10, 30, "time : %d", frame);
		Novice::ScreenPrintf(10, 50, "colortime : %d", colortime);
		Novice::ScreenPrintf(10, 70, "start : %d", start);
		Novice::ScreenPrintf(10, 110, "MP = %f", MP);
		Novice::ScreenPrintf(10, 130, "X = %d", MouseX);
		Novice::ScreenPrintf(10, 150, "Y = %d", MouseY);
		...
		if (start1 == 2) {
			...
			Novice::DrawQuad(
			  ...
			  BSX + sepalatex2, BSY + scaley + sepalatey2, BSX + scalex + sepalatex2,
			  BSY + scaley + sepalatey2, 0, 0, scalex, scaley, bigslash[BSnumber], 0xFFFFFFFF);
